...
def predicate():
    ...
    kmeans = KMeans(init="random", n_clusters=8000, random_state=0).fit(vecset_paper)     

    print("训练结束")

    predict_pred_valid = kmeans.predict(vecset_valid)

    predict_pred_paper = kmeans.predict(vecset_paper)
    ...
    for cla_valid in predict_pred_valid:
        ...
        for cla_paper in predict_pred_paper:

            
            if cla_valid==cla_paper:

                for data in train_list:
                    if data["pub_id"] == paper_list[count]["id"]:
    ...
    with open("D://data//Paper recommendation system//predicate_new.json", 'w', encoding='utf-8-sig', errors='ignore') as file:
